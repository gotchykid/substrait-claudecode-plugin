...
You are deploying the current project to **Substrait**. The deploy script picks the
path from the linked app's mode: a **GitHub-connected app** is deployed by *trigger* —
Substrait pulls the pushed branch, so nothing is uploaded and everything (including
`openapi.json` and `substrait.yaml`) must be **committed and pushed** first — while any
other app is zipped (source only) and uploaded. Either way, `--watch` follows the build
until the preview is live.
...
1b. **Offer the deploy-path choice (once).** Run
   `bash "${CLAUDE_PLUGIN_ROOT}/scripts/substrait-link.sh" modes` — it prints
   `app_mode` (the app's current path), `tenant_upload`/`tenant_connect` (what this
   workspace allows) and `chosen` (the recorded choice in `.substrait/config.json`).
     ...
     - GitHub: walk the one-time setup in order — `modes` also prints the local git
       ...
       2. **The `gh` CLI makes the rest one-command** (optional — the browser path
          ...
          - Installed but not logged in? Check `gh auth status`; if it reports no
            authentication, have the **user** run `gh auth login` — it's interactive
            (browser device-code flow), so don't run it yourself and wait: in Claude
            Code they can type `! gh auth login` to run it inside the session, or run
            it in any terminal. Re-check `gh auth status` after.
       3. **Pick the GitHub repo:** `… substrait-link.sh repos` lists the repos
          reachable through the user's Substrait GitHub App installations (rows:
          `repo<TAB>default branch<TAB>install id`). If the repo they want isn't
          listed — or doesn't exist on GitHub yet — help them create it: with `gh`
          authenticated, `gh repo create <owner>/<name> --private --source=. --push`
          creates the repo, adds `origin` and pushes in one go; without `gh` they
          create it in the browser. Then have them install the Substrait GitHub App
          on it (relay the install URL `repos` prints when empty; existing
          installations are extended from GitHub's app settings page) and re-run
          `repos` until the repo appears.
       4. **No `origin` remote** (`git_remote: none`)? Add it and push:
          ...
          credentials.
       5. **Connect:** `… substrait-link.sh set-mode --mode connect
          --repo <owner/repo> --branch <the branch you pushed>`.
     - Zip: `… substrait-link.sh set-mode --mode upload` (records the choice).
